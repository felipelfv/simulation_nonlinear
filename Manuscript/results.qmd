...
The results reported here focus primarily on conditions where nonlinear effects were present. This section is organized as follows: we first present the main results for Simulations 1 and 2 under normal measurement errors and structural disturbances, followed by a separate subsection examining how nonnormal measurement errors and structural disturbances affect results relative to the main analyses.
...
Results are presented as dumbbell plots comparing normal to nonnormal (right-skewed) conditions for measurement errors and structural disturbances. Because nonnormal measurement errors and structural disturbances can lead to additional convergence failures and outliers, the effective sample sizes may differ between conditions. To account for this, we computed the MCSE of the difference assuming independence between simulation runs:
$$
\text{MCSE}_{\text{diff}} = \sqrt{\text{MCSE}_{\text{normal}}^2 + \text{MCSE}_{\text{nonnormal}}^2}
$$

\noindent where $\text{MCSE}_{\text{normal}}$ and $\text{MCSE}_{\text{nonnormal}}$ are the MCSE for each condition. The difference between conditions and its associated uncertainty are reported in the supplemental materials. In the main text, the dumbbell plots display the normal and nonnormal condition estimates, with error bars showing ±MCSE for the nonnormal estimate. For brevity, we comment and show solely the results for the larger discrepancies observed in both simulation studies; the complete set of results is available in the supplemental materials. We also highlight that under low reliability, substantial convergence issues for the UPI approach were observed but not for the remaining approaches, which is in itself an interesting result. However, given the way we handle nonconvergence and outliers, the effective sample sizes were small. For this reason, we focus only on moderate and high reliabilities.
...
For the relative bias, focusing on moderate sample sizes, right-skewed measurement error with medium reliability lead to substantial worsening for all approaches, especially in the case with uniform latent predictors. However, under high reliability, conclusions stay largely in agreement (see Figure 12). For relative RMSE, under moderate sample sizes, the largest difference is for the UPI approach with medium reliability and right-skewed measurement error (see Figure 13). For the SE/SD ratio, results are largely similar, with the most notable difference for the UPI under right-skewed measurement error and medium reliability. In this case, we see an overestimation with right-skewed and uniform latent variables (see Figure 14). All of these results are not different under a large sample size. For coverage, results yielded the most substantial differences under medium reliability. Indeed, with right-skewed measurement error and latent variables, the coverage for LMS and QML improved. This is rather unexpected. While for right-skewed measurement error and uniform latent variables, most approaches also yielded lower coverage results except the UPI where this was mild (see Figure 15). Results for Type I error followed the behavior from coverage.

### Simulation 2

Due to the conclusions being largely the same as what happened with the simpler model in simulation 1, we do not show any figures in the manuscript. These are all the online supplemental materials. 
