...
#' Look up true parameter value using the flat naming convention
#' @param true_params List. True parameters from all_results[[i]]$true_parameters
#' @param equation Character. Equation name: "eta4" or "eta5"
#' @param param Character. Parameter name: "eta1", "eta1:eta2", etc.
#' @return Numeric. True value, or 0 if not found.
get_true_value <- function(true_params, equation, param) {
  
  # Convert "eta1:eta2" to "eta1eta2" (remove colon)
  param_clean <- gsub(":", "", param)
  # Build key: "eta4_eta1" or "eta4_eta1eta2"
  key <- paste0(equation, "_", param_clean)
  true_params[[key]] %||% 0
}

extract_data_list_study2 <- function(tbls, rep_indices, method, equation, p_name) {
  out <- lapply(rep_indices, function(rep) {
    extr <- extract_params_study_2(tbls[[rep]], method, equation)
...
extract_params_study_2 <- function(table, method, equation) {
...
#' Check for proper solution with configurable strictness (Study 2)
#' @param table Data.frame. Parameter table from estimation method output
#' @param method Character. Type of method: "lsam", "qml", or "upi"
...
check_proper_solution_study2 <- function(table, method,
  ...
  if (check_positive_se) {
    ...
    # dont count the fixed parameters (SE = 0 for LSAM/UPI, SE = NA for QML)
  ...
  if (check_pd) {
    # Study 2 has 5 latent variables
    latents <- c("eta1", "eta2", "eta3", "eta4", "eta5")
    ...
    if (all(c("lhs", "rhs", "op") %in% names(table))) {
      ...
      if (nrow(cov_rows) > 0) {
        ...
        cov_matrix <- matrix(0, 5, 5, dimnames = list(latents, latents))
...
#' Extract Convergence and Outliers with Configurable Checks (Study 2)
  ...
  for (i in seq_along(all_results)) {
    ...
    method_convergence <- setNames(lapply(methods, function(method) {
      ...
      vapply(seq_len(max_reps), function(rep) {
        ...
        if (!check_proper_solution_study2(tbls[[rep]], method,
        ...
        # basic check: both equations must have valid values
        all(sapply(names(parameters_of_interest), function(eq) {
          extr <- extract_params_study_2(tbls[[rep]], method, eq)
    ...
    for (method in methods) {
      ...
      for (eq in names(parameters_of_interest)) {
        for (p_name in parameters_of_interest[[eq]]) {
          converged_list <- extract_data_list_study2(tbls, valid_rep_indices, method, eq, p_name)
          
          if (length(converged_list) > 0) {
            tv <- get_true_value(true_params, eq, p_name)
            ...
            converged_data[[key]] <- list(
              ...
              equation = eq, parameter = p_name, true_value = tv,
          ...
          if (!is.null(extr)) {
            for (p in parameters_of_interest[[eq]]) {
              if (p %in% names(extr$Estimates)) {
                ...
                # flag replications with extreme SEs
                se_val <- extr$`Standard Errors`[p]
                if (is_valid(se_val) && se_val > se_threshold) {
                  is_outlier <- TRUE
                  break
                }
              }
    ...
    for (method in methods) {
      ...
      for (eq in names(parameters_of_interest)) {
        for (p_name in parameters_of_interest[[eq]]) {
          data_list <- extract_data_list_study2(tbls, valid_final_indices, method, eq, p_name)
          ...
          summary_list[[length(summary_list) + 1]] <- data.frame(
            ...
            Equation = eq, Parameter = p_name, TrueValue = tv, N_Total = max_reps,
...
CalculatePerformanceMetrics_Study2 <- function(filtered_data,
  ...
  for (key in names(filtered_data)) {
    ...
    results_list[[length(results_list) + 1]] <- data.frame(
      ...
      Equation = d$equation, Parameter = d$parameter, TrueValue = tv,
      ...
      CoverageRate_MCSE = cov_metrics$coverage_mcse * 100, CI_Width_MCSE = cov_metrics$width_mcse,
...
# )
#
# results_basic <- CalculatePerformanceMetrics_Study2(
...
# )
# 
# results_strict <- CalculatePerformanceMetrics_Study2(
